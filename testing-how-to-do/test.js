...
async function load_data() {
    try {
        const dataset = await d3.json("measures.json");
        var dataArray = [];
        for (var i in dataset) {
            dataArray.push({
                AirPressure: parseFloat(dataset[i].Measures.AirPressure.replace(",", ".")),
                Humidity: parseFloat(dataset[i].Measures.Humidity.replace(",", ".")),
                Temp: parseFloat(dataset[i].Measures.Temp.replace(",", ".")),
                Timestamp: new Date(dataset[i].Measures.Timestamp)
            });
        }
        return dataArray;
    } catch (error) {
